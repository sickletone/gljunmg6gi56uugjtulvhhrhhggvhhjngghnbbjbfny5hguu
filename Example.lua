local repo = "https://raw.githubusercontent.com/sickletone/gljunmg6gi56uugjtulvhhrhhggvhhjngghnbbjbfny5hguu/main/"
...
local Window = Library:CreateWindow({
	...
	-- MinSize and MaxSize (Vector2) clamp how far it can be resized
...
-- Top bar widgets: each opens a widget window and/or fires a callback.
-- Icon accepts a built-in Lucide name (see Library.LucideIcons, or add your
-- own with Library:RegisterLucideIcon) or a raw "rbxassetid://..." string.

local SettingsWidget = Library:CreateWidgetWindow({
    Title = "Settings",
    Size = UDim2.fromOffset(280, 200),
    Position = UDim2.fromOffset(300, 120)
})
SettingsWidget:AddLabel("Widget window opened from the top bar.")
SettingsWidget:AddButton({ Text = "Hello from widget", Func = function() Library:Notify("Widget button clicked") end })

local PlayersWidget = Library:CreateWidgetWindow({
    Title = "Players",
    Size = UDim2.fromOffset(260, 220),
    Position = UDim2.fromOffset(320, 140)
})
PlayersWidget:AddLabel("Player list widget.")

-- Mini windows are the same underlying sub window as widget windows, just
-- smaller and collapsible/resizable by default. Use them for compact,
-- glanceable panels rather than full settings pages.
local InfoMiniWindow = Library:CreateMiniWindow({
    Title = "Quick Info",
    Position = UDim2.fromOffset(340, 160)
})
InfoMiniWindow:AddLabel("A compact sub window.", true)
InfoMiniWindow:AddLabel("Drag the title bar to move it, the corner to resize it, and \xe2\x80\x93 to collapse it.", true)
InfoMiniWindow:AddButton({ Text = "Close", Func = function() InfoMiniWindow:Hide() end })

local SettingsTopBarWidget = Library:AddTopBarWidget({ Icon = "wrench", Tooltip = "Settings", WidgetWindow = SettingsWidget })
Library:AddTopBarWidget({ Icon = "users", Tooltip = "Players", WidgetWindow = PlayersWidget })
Library:AddTopBarWidget({ Icon = "sparkles", Tooltip = "Quick Info", WidgetWindow = InfoMiniWindow })

-- Mobile buttons
local MobileGroup = Tabs["UI Settings"]:AddRightGroupbox("Mobile")

MobileGroup:AddSlider("MobileTransparency", {
    Text = "Transparency",
    Default = Library.MobileButtonConfig.Transparency,
    Min = 0, Max = 1, Rounding = 2,
    Callback = function(v) Library:SetMobileButtonTransparency(v) end
})
MobileGroup:AddSlider("MobileWidth", {
    Text = "Width",
    Default = Library.MobileButtonConfig.Size.X,
    Min = 60, Max = 140, Rounding = 0,
    Callback = function(v) Library:SetMobileButtonSize(Vector2.new(v, Library.MobileButtonConfig.Size.Y)) end
})
MobileGroup:AddSlider("MobileHeight", {
    Text = "Height",
    Default = Library.MobileButtonConfig.Size.Y,
    Min = 22, Max = 50, Rounding = 0,
    Callback = function(v) Library:SetMobileButtonSize(Vector2.new(Library.MobileButtonConfig.Size.X, v)) end
})
MobileGroup:AddSlider("MobileCorner", {
    Text = "Corner Radius",
    Default = Library.MobileButtonConfig.CornerRadius,
    Min = 0, Max = 20, Rounding = 0,
    Callback = function(v) Library:SetMobileCornerRadius(v) end
})
MobileGroup:AddDropdown("MobileTapMode", {
    Text = "Tap Mode",
    Values = {"Single", "Double", "Hold"},
    Default = 1,
    Callback = function(v) Library:SetMobileButtonTapMode(v) end
})
MobileGroup:AddLabel("Mobile Color"):AddColorPicker("MobileColor", {
    Default = Library.MobileButtonConfig.Color,
    Callback = function(v) Library:SetMobileButtonColor(v) end
})
MobileGroup:AddToggle("MobileInvisible", {
    Text = "Invisible",
...
TopBarGroup:AddSlider("TopBarIconSize", {
    Text = "Icon Size",
    Default = Library.TopBarIconSize,
    Min = 12, Max = 40, Rounding = 0,
    Callback = function(v) Library:SetTopBarIconSize(v) end
})
TopBarGroup:AddSlider("TopBarTransparency", {
    Text = "Transparency",
    Default = Library.TopBarTransparency,
    Min = 0, Max = 1, Rounding = 2,
    Callback = function(v) Library:SetTopBarTransparency(v) end
})
TopBarGroup:AddLabel("Icon Color"):AddColorPicker("TopBarIconColor", {
    Default = Library.FontColor,
    Callback = function(v) Library:SetTopBarIconColor(v) end
})
TopBarGroup:AddLabel("Background Color"):AddColorPicker("TopBarBackgroundColor", {
    Default = Library.BackgroundColor,
    Callback = function(v) Library:SetTopBarBackgroundColor(v) end
})
TopBarGroup:AddDropdown("TopBarSettingsIcon", {
    Text = "Settings Icon",
    Values = {"wrench", "cog", "sliders", "settings"},
    Default = 1,
    Callback = function(v) Library:SetTopBarWidgetIcon(SettingsTopBarWidget, v) end
})

-- Glass material: frosted, translucent surfaces on the window, its sub
-- windows, and the top bar. The "Liquid Glass" theme (Themes groupbox
-- below) turns this on automatically; these let you control it directly.
local GlassGroup = Tabs["UI Settings"]:AddRightGroupbox("Glass")

GlassGroup:AddToggle("GlassEnabled", {
    Text = "Enabled",
    Default = Library.GlassEnabled,
    Callback = function(v) Library:SetGlassEnabled(v) end
})
GlassGroup:AddSlider("GlassIntensity", {
    Text = "Intensity",
    Default = Library.GlassIntensity,
...
LayoutGroup:AddSlider("DPISlider", {
    Text = "DPI",
    Default = 100,
    Min = 50, Max = 150, Rounding = 0, Suffix = "%",
    Callback = function(v) Library:SetDPI(v) end
})
LayoutGroup:AddDropdown("TabPosition", {
    Text = "Tab Position",
...
LayoutGroup:AddButton({ Text = "Set Background", Func = function()
    local id = Options.CustomBGInput.Value
    if id and id ~= "" then
        Library:SetCustomBackground(id)
        Library:Notify("Background set")
    end
